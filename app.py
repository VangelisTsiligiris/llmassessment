...
def render_quill_html(key: str, initial_html: str) -> str:
    """Robust wrapper for streamlit-quill (works across versions)."""
    try:
...
if APP_PASSCODE and not st.session_state["__auth_ok"]:
    ...
    with colB:
        sid = st.text_input("Your User ID", value=st.session_state.get("user_id") or "")
        if st.button("Generate new ID"):
            sid = _gen_id()
            st.session_state["user_id"] = sid
            st.info(f"Your new ID is **{sid}** — copy it.")
            st.rerun()
    if st.button("Enter"):
...
def load_secrets():
    ...
    if isinstance(sa_info, str):
        ...
        except Exception: st.error("Invalid service account JSON"); st.stop()
...
if genai is None or not GEMINI_KEY:
    st.error("Gemini not available or missing key."); st.stop()
...
def _get_or_create_ws(title, headers):
    try: return sh.worksheet(title)
        ...
        ws.append_row(headers, value_input_option="USER_ENTERED"); return ws
...
if "draft_html" not in st.session_state: st.session_state.draft_html = ""
if "report" not in st.session_state: st.session_state.report = None
if "last_saved_at" not in st.session_state: st.session_state.last_saved_at = None
if "last_autosave_at" not in st.session_state: st.session_state.last_autosave_at = None
if "last_saved_html" not in st.session_state: st.session_state.last_saved_html = ""
...
def ask_llm(prompt_text: str):
    ...
    return "".join(chunks), round((time.time() - start) * 1000)
...
def load_progress(user_id, assignment_id):
    try:
        ...
        for r in reversed(records):
            if r.get("user_id","") == user_id and r.get("assignment_id","") == assignment_id:
                return r.get("draft_html") or ""
    except Exception: return ""
    return ""

def maybe_autosave(draft_html, draft_text):
    now = time.time(); last_ts = st.session_state.last_autosave_at or 0
    changed = (draft_html or "") != (st.session_state.last_saved_html or "")
    if changed and (now - last_ts) >= AUTO_SAVE_SECONDS:
        save_progress(st.session_state.user_id, st.session_state.assignment_id, draft_html, draft_text, True)
        st.session_state.last_autosave_at = now

def compute_similarity_report(final_text, llm_texts, sim_thresh=SIM_THRESHOLD):
    finals = segment_paragraphs(final_text); llm_segs = [s for t in llm_texts for s in segment_paragraphs(t)]
    if not finals or not llm_segs: return {"backend": SIM_BACKEND, "mean":0.0, "high_share":0.0, "rows":[]}
    rows, high_tokens = [], 0; total_tokens = sum(len(s.split()) for s in finals)
    if SIM_BACKEND == "sbert":
        ...
        F,L = vectorizer.transform(finals), vectorizer.transform(llm_segs)
    ...
    return {"backend":SIM_BACKEND,"mean":mean_sim,"high_share":round(high_tokens/max(1,total_tokens),3),"rows":rows[:30]}
...
def export_evidence_docx(user_id, assignment_id, chat, draft_html, report):
    ...
    d = docx.Document(); d.add_heading("Coursework Evidence Pack",0)
    d.add_paragraph(f"User ID: {user_id}"); d.add_paragraph(f"Assignment ID: {assignment_id}")
    d.add_paragraph(f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    d.add_heading("Chat with LLM",1)
    for m in chat:
        role = "Student" if m["role"]=="user" else "LLM"; d.add_paragraph(f"{role}: {m['text']}")
    d.add_heading("Final Draft",1)
    for para in final_text.split("\n"): d.add_paragraph(para)
    d.add_heading("Similarity Report",1)
    d.add_paragraph(f"Backend: {report.get('backend','-')}"); d.add_paragraph(f"Mean: {report.get('mean',0.0)}")
    d.add_paragraph(f"High-sim share: {report.get('high_share',0.0)*100:.1f}%")
    for r in report.get("rows",[]): d.add_paragraph(f"- Cos: {r['cosine']} | Final: {r['final_seg']} | LLM: {r['nearest_llm']}")
...
with left:
    ...
    if send and prompt.strip():
        ...
        log_event("chat_user",prompt,""); log_event("chat_llm",prompt,reply); st.rerun()
...
with right:
    ...
    k1.metric("Words",word_count(plain)); k2.metric("Characters",char_count(plain)); k3.metric("LLM Responses",len(st.session_state.llm_outputs))
    maybe_autosave(st.session_state.draft_html,plain)
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("💾 Save draft"): save_progress(st.session_state.user_id,st.session_state.assignment_id,st.session_state.draft_html,plain,False)
    with c2:
        if st.button("📊 Run similarity"):
            if plain.strip() and st.session_state.llm_outputs:
                rep=compute_similarity_report(plain,st.session_state.llm_outputs,SIM_THRESHOLD); st.session_state.report=rep
            ...
            else: st.warning("Need draft text + one LLM response first.")
    with c3:
